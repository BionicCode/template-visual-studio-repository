...
Invoke-Test "Workflow preserves repair design and deterministic branch hash" {
    ...
    Assert-True ($workflow -match "workflow_call:") "Workflow should expose workflow_call for the repository-maintenance orchestrator."
    Assert-True ($workflow -match "workflow_dispatch:") "Workflow should keep workflow_dispatch for manual debugging."
    ...
    Assert-True ($workflow -match 'concurrency:\s*[\r\n]+\s+group:\s*document-metadata-\$\{\{ inputs\.event_name \|\| github\.event_name \}\}-\$\{\{ inputs\.head_repository \|\| github\.repository \}\}-\$\{\{ inputs\.head_ref \|\| inputs\.ref_name \|\| github\.ref \}\}\s*[\r\n]+\s+cancel-in-progress:\s*true') "Workflow should cancel stale runs for the same effective event/repository/ref."
    Assert-True ($workflow -notmatch '\$\{\{ github\.workflow \}\}-\$\{\{ github\.event\.pull_request\.number \|\| github\.ref \}\}') "Workflow should not use the pre-orchestrator concurrency contract."
    ...
    Assert-True ($workflow -match 'analyze-document-metadata:[\s\S]*?permissions:\s*[\r\n]+\s+contents: read') "Analyze job should use contents: read."
    Assert-True ($workflow -match 'repair-document-metadata:[\s\S]*?permissions:\s*[\r\n]+\s+contents: write\s*[\r\n]+\s+pull-requests: write') "Repair job should have write permissions only in repair job."
    ...
    Assert-True ($workflow -match 'repository:\s*\$\{\{ inputs\.head_repository \|\| github\.repository \}\}') "Workflow should fall back to the current repository when head_repository is empty."
    Assert-True ($workflow -match '\(inputs\.head_repository \|\| github\.repository\) == github\.repository') "Repair guard should fall back to the current repository for manual same-repository simulations."
    Assert-True ($workflow -match "startsWith\(inputs\.head_ref, 'codex/doc-metadata-repair/'\)" -and $workflow -match "startsWith\(inputs\.ref_name \|\| github\.ref_name, 'codex/doc-metadata-repair/'\)") "Workflow should guard recursive doc-metadata repair branches using forwarded inputs."
    ...
    Assert-True ($workflow -match 'Join-Path \$env:RUNNER_TEMP "doc-metadata-repair-paths\.txt"') "Workflow should keep repair path-list helper files outside the repository working tree."
    Assert-True ($workflow -match 'Join-Path \$env:RUNNER_TEMP "doc-metadata-links\.json"') "Workflow should keep generated link-map helper files outside the repository working tree."
    Assert-True ($workflow -match 'Join-Path \$env:RUNNER_TEMP "doc-metadata-repair-summary\.md"') "Workflow should keep generated PR body helper files outside the repository working tree."
    ...
    Assert-True ($workflow -match "Incremented files" -and $workflow -match "Initialized files" -and $workflow -match "Metadata repair files" -and $workflow -match "Skipped files" -and $workflow -match "Remaining failed files" -and $workflow -match "Remaining unrecoverable files") "Repair PR body should include mutually exclusive changed-file categories plus skipped, failed, and unrecoverable sections."
