...
10. Add a job that calls `.github/workflows/doc-metadata.yml` first when the normalized context says it applies. Pass explicit inputs instead of depending on the callee to inspect the parent event payload implicitly, and preserve the existing `final-status-report` semantics and job identity unless reusable-workflow conversion forces a minimal adjustment.
...
12. Implement the sync call job with a job-level `if` built around `!cancelled()` plus normalized outputs from the orchestration context and the doc-metadata call result.
13. The sync guard must allow execution only when `normalize.outputs.should_run_sync_managed_files == 'true'` and one of these is true: doc-metadata was not expected to run (`normalize.outputs.should_run_doc_metadata != 'true'`), or doc-metadata was expected and completed successfully. The guard must not allow sync merely because `needs.<doc-metadata-job>.result == 'skipped'`.
14. If doc-metadata was expected to run, sync must not run when the doc-metadata call job is skipped, fails, or is cancelled.
15. Keep schedule ownership only in `repository-maintenance.yml`; remove schedule ownership from `sync-managed-files.yml`.
...
4. Verify the sync job-level `if` uses `!cancelled()` plus normalized outputs and the doc-metadata call result.
5. Verify the sync guard allows execution only when sync is in scope and either doc-metadata was out of scope or doc-metadata completed successfully.
6. Verify the orchestrator does not run sync when doc-metadata was expected to run but the doc-metadata call job is skipped, fails, or is cancelled.
7. Verify schedule ownership exists only in the orchestrator and no longer in the child workflows.
8. Verify direct manual runs still work on both child workflows via `workflow_dispatch`, and orchestrator manual runs trigger the centralized path.
9. Verify the local sync workflow still uses `BionicCode/workflows/.github/workflows/sync-files-from-manifest.yml@main` and remains a wrapper rather than inlining shared logic.
10. Verify `doc-metadata.yml` preserves the existing `final-status-report` semantics and job identity unless a minimal reusable-workflow compatibility adjustment is demonstrably required.
...
- Use normalized orchestrator outputs plus a `!cancelled()` sync job guard so `needs` preserves ordering without suppressing sync on intentional doc-metadata out-of-scope cases.
- Do not treat `needs.<doc-metadata-job>.result == 'skipped'` as sufficient to run sync; allow that path only when `normalize.outputs.should_run_doc_metadata != 'true'`.
- Preserve the existing doc-metadata `final-status-report` semantics without renaming job ids unless reusable-workflow conversion requires a minimal change.
...
2. The second highest-risk detail is `needs` result handling in the orchestrator. Implementation should normalize whether doc-metadata was in scope and combine that with the doc-metadata job result so sync tolerates only true out-of-scope skips, not expected-but-skipped runs.
